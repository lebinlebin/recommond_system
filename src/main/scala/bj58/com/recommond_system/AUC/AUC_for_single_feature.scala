...
class AUC_for_single_feature {
}
object AUC_for_single_feature{
    def main(args: Array[String]) {
        val conf = new SparkConf().setAppName("test").setMaster("local")
        val sc = new SparkContext(conf)
        LogisticRegressionDataGenerator
        //数据生成使用LogisticRegressionDataGenerator，可以用过MLUtils快速加载数据
        val trainData: RDD[LabeledPoint] = MLUtils.loadLabeledPoints(sc,"data/AUCtrain")
        val testData: RDD[LabeledPoint] = MLUtils.loadLabeledPoints(sc,"data/AUCtest")
        //模型训练
        ...
        val indexSum = scores.map(x=>{
            if(x._1._2.equals("1.0")){
                x._2  //RDD[((Double, Double), Long)]  取第二项
            }else{
                0
            }
        }).sum()
